...
/* ═══════════════════════════════════════════════════════════════════════════
   Feature3DScene — contextual 3D objects for the four pillars
   Each shape is purpose-built to represent its concept.
   ═══════════════════════════════════════════════════════════════════════════ */
...
function SearchMesh({ color, emissive }: { color: string; emissive: string }) {
  ...
  useFrame((state, delta) => {
    ...
    if (dotsRef.current) {
      dotsRef.current.rotation.y -= delta * 0.6
    }
  })
  ...
  return (
    <Float speed={1.2} rotationIntensity={0.15} floatIntensity={0.25}>
      <group ref={groupRef}>
        ...
        {/* Orbiting data dots — discovery in action */}
        <group ref={dotsRef}>
          {[0, 1, 2, 3, 4, 5].map((i) => {
...
function ShieldMesh({ color, emissive }: { color: string; emissive: string }) {
  ...
  // Checkmark — built as a tube so it avoids the SVG <line> conflict
...
function LedgerMesh({ color, emissive }: { color: string; emissive: string }) {
  ...
  useFrame((state, delta) => {
    if (ref.current) {
      ...
      ref.current.rotation.x = Math.sin(state.clock.elapsedTime * 0.35) * 0.1
    }
  })
    ...
    <div style={{ width: size, height: size }}>
      ...
      >
        <Suspense fallback={null}>
          ...
          <directionalLight position={[4, 4, 5]} intensity={0.7} color="#F0FFF8" />
          <directionalLight position={[-2, -1, 3]} intensity={0.3} color="#D8F7ED" />
          <pointLight position={[2, 1, 3]} intensity={0.2} color="#A5EBD3" />
